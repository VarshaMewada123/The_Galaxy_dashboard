...
const Offers = () => {
  ...
  const staticOffers = [
    {
      ...
      validFrom: 'Mar 01',
      validTo: 'May 31',
      description: 'Experience luxury with 25% OFF on all room bookings for 2+ nights including breakfast.',
      bookings: 124,
      revenue: 245000,
    ...
    },
    {
      ...
      validFrom: 'Feb 01',
      validTo: 'Apr 30',
      description: 'Celebrate love with 35% OFF on Premium Suites and a complimentary candle-lit dinner.',
      bookings: 89,
      revenue: 356000,
      image: 'https://images.unsplash.com/photo-1578683015141-39940e9653ec?w=400&fit=crop'
    },
    {
      id: 3,
      ...
      validFrom: 'Feb 11',
      validTo: 'Dec 31',
      description: 'Relish our grand buffet with a 15% discount for our in-house premium guests.',
      bookings: 342,
      revenue: 178000,
      image: 'https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=400&fit=crop'
    }
  ];
  ...
  const filteredOffers = offers.filter(offer => {
    ...
    const matchesFilter = filterStatus === 'all' || offer.status === filterStatus || offer.type === filterStatus;
    ...
    <div className="space-y-8 p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
        {filteredOffers.map((offer) => (
          <div key={offer.id} className="group bg-white border border-gray-100 rounded-sm overflow-hidden hover:shadow-xl transition-all duration-500">
            {/* Image Wrap */}
            <div className="relative h-56 overflow-hidden">
              ...
              <div className="absolute bottom-4 right-4 bg-[#C6A45C] text-white w-14 h-14 rounded-full flex flex-col items-center justify-center shadow-lg border-2 border-white/20">
                <span className="text-xs font-bold leading-none">{offer.discount}%</span>
            ...
            <div className="p-6 space-y-4">
              <div className="flex justify-between items-start">
                ...
                <span className={`text-[9px] uppercase font-bold tracking-tighter px-2 py-1 rounded-sm ${
                  offer.status === 'active' ? 'bg-green-50 text-green-600' : 'bg-gray-50 text-gray-400'
                }`}>
                  {offer.status}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 py-4 border-y border-gray-50">
                <div className="flex items-center gap-2 text-gray-400">
                  ...
                  <span className="text-[10px] uppercase tracking-wider font-semibold">{offer.validFrom} - {offer.validTo}</span>
                </div>
                <div className="flex items-center gap-2 text-gray-400">
                  <Users size={14} className="text-[#C6A45C]" />
                  <span className="text-[10px] uppercase tracking-wider font-semibold">{offer.bookings} Booked</span>
                </div>
              </div>

              <div className="flex items-center justify-between pt-2">
                <div>
                  <p className="text-[10px] text-gray-400 uppercase tracking-widest line-through">₹{offer.originalPrice}</p>
                  <p className="text-2xl font-serif text-gray-900">
                    ₹{(offer.originalPrice * (1 - offer.discount / 100)).toLocaleString()}
                  </p>
                </div>
      ...
      {filteredOffers.length === 0 && (
        <div className="text-center py-24 border border-dashed border-gray-100 rounded-sm">
          ...
          <h3 className="font-serif text-2xl text-gray-800 uppercase tracking-widest">No Offers Available</h3>
          <p className="text-gray-400 text-sm mt-2 font-medium tracking-widest">PERHAPS IT'S TIME TO CREATE A NEW CAMPAIGN</p>
        </div>
      )}
    </div>
  );
