      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 lg:py-12">
        ...
        <header className="flex flex-col sm:flex-row sm:items-end justify-between mb-10 gap-4">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <h1 className="text-4xl font-light tracking-tight text-slate-900">
              Menu <span className="font-serif italic text-[#C5A059] font-medium">Categories</span>
            </h1>
          </motion.div>
          ...
          <div className="inline-flex items-center bg-white border border-slate-200 px-4 py-2 rounded-xl shadow-sm">
            <LayoutGrid size={14} className="text-[#C5A059] mr-2" />
            <p className="text-slate-500 text-[10px] font-bold tracking-widest uppercase">
              Live Sections: <span className="text-slate-900 ml-1">{categories.length}</span>
            </p>
          </div>
        </header>
            ...
            <div className="flex justify-end gap-2 border-t border-slate-50 pt-2">
              {editing && (
                <button
                  type="button"
                  onClick={resetForm}
                  className="px-4 py-2 text-slate-400 hover:text-red-500 text-[11px] font-bold uppercase tracking-widest transition-colors"
                >
                  Cancel
                </button>
              )}
              <button
                ...
                className="whitespace-nowrap flex items-center justify-center gap-2 px-8 py-3 rounded-xl bg-[#C5A059] text-white hover:bg-[#2D2D2D] transition-all duration-300 shadow-md disabled:opacity-50 active:scale-95"
              >
                {createMutation.isPending || updateMutation.isPending ? (
                  <Loader2 className="w-4 h-4 animate-spin" />
                ) : (
          ...
          <AnimatePresence mode="popLayout">
            {categories.map((cat, index) => (
              <motion.div
                ...
                animate={{ opacity: 1, scale: 1 }}
                exit={{ opacity: 0, scale: 0.9 }}
                className="group bg-white border border-slate-200/60 rounded-2xl overflow-hidden hover:border-[#C5A059]/40 hover:shadow-xl transition-all duration-300 relative"
              >
                {/* Category Image */}
                <div className="relative h-40 w-full overflow-hidden bg-slate-100">
                  ...
                  <div className="absolute top-3 left-3 flex items-center gap-1 bg-black/20 backdrop-blur-md px-2 py-1 rounded-md">
                  ...
                  <div className="flex items-center gap-2 mt-auto pt-4 border-t border-slate-50">
                    <button
                      onClick={() => {
                        setEditing(cat._id);
                        setName(cat.name);
                        // window.scrollTo usually isn't great for UX, but kept as per original
                        window.scrollTo({ top: 0, behavior: "smooth" });
                      }}
                      className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-slate-50 text-slate-500 hover:bg-[#C5A059] hover:text-white rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all"
                    >
        ...
        {/* Empty State */}
        {categories.length === 0 && !isLoading && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-slate-200 rounded-[2rem] bg-white/50">
            <ChefHat size={40} className="text-slate-200 mb-4" />
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest">No Categories Found</h3>
          </motion.div>
        )}
      </div>
    </div>
